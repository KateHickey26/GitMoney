...
*/
			...
			numberOfPlayers = promptUserInput("\nHow many AI players do you want in the game?\n" +
					"Minimum of 1 and maximum of 4 players permitted.\n" +
					"Enter the number of players: ", new int[]{1, 2, 3, 4});
		...
		//############	System.out.println("size of deck (should be zero):"+game.getMainDeck().sizeOfDeck());
			...
			System.out.println("\n\nGame Start");
			updateTestLogFile("TestLog.txt", "Game Start at "+new Timestamp(System.currentTimeMillis())+"\n");
			...
			while (!gameOver) {
				...
				while (!humanIsActivePlayer) {
					...
					updateTestLogFile("TestLog.txt", "\nRound "+roundCounter+": "+
							game.getPlayer().get(game.getActivePlayer()).getName()+" is active player ###############");
					...
					for (int k = 0; k < game.getPlayer().size(); k++) {
						updateTestLogFile("TestLog.txt",
								game.getPlayer().get(k).getName()+" has "+
										game.getPlayer().get(k).getDeck().sizeOfDeck()+" cards. "+
								"Top card is "+game.getPlayer().get(k).getDeck().getTopCard().toString());
					}
				...
				//#######	System.out.println("Maindeck size at creation:" + game.getMainDeck().sizeOfDeck());
					...
					if(roundWinner==-1){
						System.out.println("Round "+roundCounter+": This round was a Draw, common pile now has "+game.getCommunalDeck().sizeOfDeck()+" cards");
					}
